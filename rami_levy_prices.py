...
def find_rami_folder():
    ...
    folders = [os.path.join("dumps", f) for f in os.listdir("dumps") if os.path.isdir(os.path.join("dumps", f))]
...
def extract_and_show(folder):
    ...
    for f in sorted(os.listdir(folder)):
        ...
        tag = "ПОЛНЫЙ" if "Full" in f and f.endswith(".xml") else "частичный" if f.endswith(".xml") else "gz"
        print(f"  [{tag}] {f}: {size:,} байт")
        if tag == "ПОЛНЫЙ": full += 1
    return full
